...
// เตือนที่ 3 จังหวะ: ก่อนถึงกำหนด 1 วัน / 1 ชั่วโมง / 30 นาที
const NOTIFY_MILESTONES = [
  { key: '1d', minutes: 24 * 60, label: 'อีก 1 วัน' },
  { key: '1h', minutes: 60, label: 'อีก 1 ชั่วโมง' },
  { key: '30m', minutes: 30, label: 'อีก 30 นาที' },
];
...
function checkDueSoonNotifications() {
  ...
  pending.forEach((a) => {
    ...
    if (minutesLeft < 0) return; // เลยกำหนดไปแล้ว ไม่ต้องเตือนซ้ำ

    const notifiedForThis = notifiedMilestones[a.id] || [];

    // หา milestone ที่ "เข้าเงื่อนไขแล้ว" (เหลือเวลาน้อยกว่าหรือเท่ากับ threshold)
    // และยังไม่เคยเตือน โดยเรียงจากใกล้เดดไลน์ที่สุด (นาทีน้อยสุด) ไปไกลสุด
    const applicable = NOTIFY_MILESTONES
      .filter((m) => minutesLeft <= m.minutes && !notifiedForThis.includes(m.key))
      .sort((x, y) => x.minutes - y.minutes);

    if (applicable.length) {
      // ยิงแจ้งเตือนแค่ตัวที่ใกล้เดดไลน์ที่สุด (ตรงกับเวลาจริงมากที่สุด)
      const closest = applicable[0];
      new Notification(`ใกล้ถึงกำหนดส่งงาน (${closest.label})`, {
        body: `${a.title} (${a.subject?.name || ''}) — ${formatThaiDate(a.dueDate)}`,
        icon: '',
      });

      // milestone อื่นที่เข้าเงื่อนไขพร้อมกัน (ไกลกว่า) ถือว่า "ผ่านช่วงเวลาไปแล้ว"
      // ให้ทำเครื่องหมายว่าเตือนแล้วเงียบ ๆ จะได้ไม่เด้งซ้ำทีหลัง
...
function startAuthListening() {
  window.sungluiAuth.onAuthChange((user) => {
    if (user) {
      ...
      refreshIntervalId = setInterval(loadAll, 60000); // รีเฟรชข้อมูลทุก 1 นาที
    } else {
      if (refreshIntervalId) clearInterval(refreshIntervalId);
      state.subjects = [];
      state.assignments = [];
